...
<body>
  <div class="container mt-4 ">
    <div class="p-3 rounded bg-light border shadow-sm">
      <h2 class="text-break">Edit Post</h2>
      <form action="/edit-post/{{$post->id}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        <div class="mb-3">
          <label for="title" class="form-label text-break">Title</label>
          <input type="text" id=title name="title" class="form-control" value="{{$post->title}}" required>
        </div>

        <div class="mb-3">
          <label for="body" class="form-label"></label>
          <textarea id="body" name="body" class="form-control" rows="5" required>{{$post->body}}</textarea>
        </div>
